...
void main()
{
	vec4 diffuse_color = texture(sprite, vec3(fs_in.tc, 0));
	vec3 normal = texture(normal_map, vec3(fs_in.tc, 0)).rgb * 2.0 - 1.0;
	for(int i = 0; i < 4; i++) {
		vec4 layer_color = texture(sprite, vec3(fs_in.tc, i + 1));
		if(side_indices[i] != 0 && layer_color.a > 0) {
			diffuse_color = layer_color;
			normal = texture(normal_map, vec3(fs_in.tc, i + 1)).rgb * 2.0 - 1.0;
			if(layer_color == vec4(1, 0, 1, 1)) {
				diffuse_color.a = 0;
			}
		}
	}
